document.addEventListener("DOMContentLoaded", () => {
  ...
  // Firefox's browser.storage.local already returns Promises natively.
  // Chrome's chrome.storage.local uses callbacks, so we wrap it when needed.
  const storageGet = (defaults) =>
    browserAPI === chrome && typeof browser === "undefined"
      ? new Promise((resolve) => chrome.storage.local.get(defaults, resolve))
      : browserAPI.storage.local.get(defaults);
